...
data <- read_excel("dataset/data.xlsx")
...
#export remesas
tiff("graficos/Figura1_Historico_Remesas.tif", units="in", width=6.5, height=4, res=300, compression="lzw")
print(p_remesas)
dev.off()

#export itcer
tiff("graficos/Figura2_Historico_ITCER.tif", units="in", width=6.5, height=4, res=300, compression="lzw")
print(p_itcer)
dev.off()
...
#fuente
#las fuente era windowsFonts(TNR = windowsFont("Times New Roman"))
#se elimino porque no se encuentra esa funcion
...
#export de graficos
tiff("graficos/Figura3_ITCER_logniveles.tif", units="in", width=6.5, height=4, res=300, compression="lzw")
print(p_itcer_nexo)
dev.off()

tiff("graficos/Figura4_Remesas_logniveles.tif", units="in", width=6.5, height=4, res=300, compression="lzw")
print(p_remesas_nexo)
dev.off()

tiff("graficos/Figura5_Transable_logniveles.tif", units="in", width=6.5, height=4, res=300, compression="lzw")
...
#test de cointegracion estructural

#variables en niveles (logaritmos) que están en data_final
data_johansen <- data_final %>%
  dplyr::select(ipi_eeuu, iti, remesas, itcer, transable)

# Convertir a matriz
data_johansen_mat <- as.matrix(data_johansen)


#quiebre de las dummys
# se puede cambiar el trimestre del quiebre si se desea
quiebre_trimestre <- which(data_final$dummy_2020 == 1)[1]

# lag = 7
johansen_quiebre <- ca.jo(data_johansen_mat, type = "trace", ecdet = "const", K = 7, dumvar = exogenas)

summary(johansen_quiebre)
...
tiff("graficos/Figura8a_IRF_Remesas_ITCER.tif", units="in", width=6.5, height=4, res=300, compression="lzw")
#tiff("graficos/Figura8a_IRF_Remesas_ITCER_2do_orden.tif", units="in", width=6.5, height=4, res=300, compression="lzw")
print(p_itcer_nexo)
dev.off()

tiff("graficos/Figura8b_IRF_Remesas_Transable.tif", units="in", width=6.5, height=4, res=300, compression="lzw")
#tiff("graficos/Figura8b_IRF_Remesas_Transable_2do_orden.tif", units="in", width=6.5, height=4, res=300, compression="lzw")
print(p_transable_nexo)
dev.off()

tiff("graficos/Figura8c_IRF_ITCER_Transable.tif", units="in", width=6.5, height=4, res=300, compression="lzw")
#tiff("graficos/Figura8c_IRF_ITCER_Transable_2do_orden.tif", units="in", width=6.5, height=4, res=300, compression="lzw")
...
#bsvars devuelve valores ajustados
...
tabla_diag <- diag_results %>%
  gt() %>%
  tab_header(title = "Pruebas de Diagnostico de Residuales (BVECM)") %>%
...
tiff("graficos/raices_bvecm.tif", units="in", width=5, height=5, res=300, compression="lzw")
print(grafico_raices_bvecm)
dev.off()
...
#grafica global
diag_plot_final <- diag_plot_coef & theme(
  plot.title    = element_text(face = "bold", size = 16, hjust = 0.5),
  plot.subtitle = element_text(size = 12, hjust = 0.5, color = "gray40"),
